...
$value= $formattedDate;;
	...
	{
		if($value>1){
	$value=$value." Vacancias ";
}
if($value==1){
	$value=$value." Vacancia ";
}
if($value<1){
	$value=$value." Vacancias";
};
		return $value;
				...
				if($table=="bolsa_empleo.vacancia" && $field=="id_estado_vacancia" && $ptype=="list")
	{
		if ($data["id_estado_vacancia"] == '1') {  
...
$result_query = DB::Query("SELECT id_estado_postulacion, 
CASE
    WHEN id_estado_postulacion = 1 THEN 'Preseleccionado'
    WHEN id_estado_postulacion = 2 THEN 'Seleccionado'
    ELSE 'Evaluacion-Empresa'
END AS descripcion
FROM bolsa_empleo.estado_postulacion WHERE id_estado_postulacion IN(1,2,3)");
...
.form-horizontal {
	...
	/*flex-wrap: wrap;*/ /* Ahora permite saltos de línea en pantallas pequeñas */
	margin-bottom: 20px; /* Espaciado inferior */
	justify-content: space-between; /* Distribuye mejor los elementos */
}

/* ------------------------- */
/* 📌 Estilo para Input de Archivos */
/* ------------------------- */
.custom-file-input {
...
select.form-control {
	...
	max-width: 100%; /* Ahora se ajusta al contenedor */
...
@media (max-width: 768px) { /* 📱 Tablets y móviles grandes */
	...
	select.form-control {
		width: 100%; /* El select ocupa el ancho completo */
	}
}
...
$sql = DB::PrepareSQL("select descripcion_cargo from bolsa_empleo.cargo where id_cargo = ':1'", $data["id_cargo"]);
$rs = DB::Query($sql);
$dataf = $rs->fetchAssoc();
$value = $dataf["descripcion_cargo"]   ;
		return $value;
	}
				if($table=="bolsa_empleo.postulacion" && $field=="sexo")
	{
		if ($data["sexo"] == 'M') {
	$value = "Masculino";
} elseif ($data["sexo"] == 'm') {  
	...
	$value = "Indefinidido";
		...
		return $value;
				...
				if($table=="vacancia_pendiente" && $field=="id_estado_vacancia" && $ptype=="list")
	{
		if ($data["id_estado_vacancia"] == '1') {  
     $value="<span class='badge badge-pendiente'>" .$value . "</span>"; 
}
...
if ($data["id_estado_vacancia"] > '6') {  
     $value="<span class='badge badge-defecto'>" .$value . "</span>";  
		...
		return $value;
				...
				if($table=="bolsa_empleo.seguimientos" && $field=="id_estado_anterior")
	{
		;
		return $value;
	}
				if($table=="bolsa_empleo.seguimientos" && $field=="id_nuevo_estado")
	{
		/*
if ($data["id_nuevo_estado"] == '1') {  
	$value = "<span class='badge badge-rechazado'>" . $value . "</span>";  //Rojo medio claro
}

if ($data["id_nuevo_estado"] == '2') {  
	$value = "<span class='badge badge-pendiente'>" . $value . "</span>"; //caqui
}

if ($data["id_nuevo_estado"] == '3') {  
	$value = "<span class='badge badge-busqueda'>" . $value . "</span>";  //Verde
}
*/

if ($data["id_nuevo_estado"] == 'Preseleccionado') {  
	$value = "<span class='badge badge-rechazado'>" . $value . "</span>";  //Rojo medio claro
}

if ($data["id_nuevo_estado"] == 'Seleccionado') {  
	$value = "<span class='badge badge-pendiente'>" . $value . "</span>"; //caqui
}

if ($data["id_nuevo_estado"] == 'Evaluacion-Empresa') {  
	$value = "<span class='badge badge-busqueda'>" . $value . "</span>";  //Verde
};
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="bolsa_empleo.vacancia" && $field=="otro_tipo_remuneracion")
	{
		return 8;
	}
				if($table=="bolsa_empleo.vacancia" && $field=="fecha_expiracion_vacancia")
	{
		return $fecha = date("Y-m-d H:i:s", strtotime("+8 days 15:00:00"));
$fecha;
	}
				if($table=="bolsa_empleo.vacancia" && $field=="cantidad_vacancia")
	{
		return 1;
				...
				if($table=="bolsa_empleo.vacancia" && $field=="edad_rango_bajo")
	{
		return 18;
	}
				if($table=="bolsa_empleo.vacancia" && $field=="anos_experiencia_vacancia")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia" && $field=="meses_experiencia_vacancia")
	{
		return 0;
	}
				if($table=="bolsa_empleo.vacancia" && $field=="modalidad_trabajo")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_dias_horas" && $field=="vacancia_dias")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_dias_horas" && $field=="vacancia_horas")
	{
		return "08:00-18:00";
	}
				if($table=="bolsa_empleo.vacancia_requisito_idioma" && $field=="id_nivel_idioma_habla")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_requisito_idioma" && $field=="id_nivel_idioma_escribe")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_requisito_idioma" && $field=="id_nivel_idioma_lee")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_requisito_programa" && $field=="id_nivel_programa")
	{
		return 1;
	}
				if($table=="vacancia_pendiente" && $field=="otro_tipo_remuneracion")
	{
		return 8;
	}
				if($table=="vacancia_pendiente" && $field=="fecha_expiracion_vacancia")
	{
		return $fecha = date("Y-m-d H:i:s", strtotime("+30 days 15:00:00"));
$fecha;
	}
				if($table=="vacancia_pendiente" && $field=="cantidad_vacancia")
	{
		return 1;
	}
				if($table=="vacancia_pendiente" && $field=="anos_experiencia_vacancia")
	{
		return 1;
	}
				if($table=="vacancia_pendiente" && $field=="meses_experiencia_vacancia")
	{
		return 0;
	}
				if($table=="vacancia_pendiente" && $field=="modalidad_trabajo")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="fecha_creacion_vacancia")
	{
		return now();
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="id_estado_vacancia")
	{
		return 2;
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="fecha_expiracion_vacancia")
	{
		return $fecha = date("Y-m-d H:i:s", strtotime("+10 days 15:00:00"));;
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="anos_experiencia_vacancia")
	{
		return 0;
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="meses_experiencia_vacancia")
	{
		return 0;
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="edad_minimo")
	{
		return 18;
	}
				if($table=="bolsa_empleo.vacancia_tmp" && $field=="edad_maximo")
	{
		return 75;
	}
				if($table=="bolsa_empleo.vacancia_dias_horas_tmp" && $field=="vacancia_dias")
	{
		return 1;
	}
				if($table=="bolsa_empleo.vacancia_dias_horas_tmp" && $field=="vacancia_horas")
	{
		return "08:00-18:00";
	}
...
function GetAutoUpdateValue($field, $ptype, $table="")
{
				...
				if($table=="bolsa_empleo.vacancia_dias_horas" && $field=="vacancia_horas")
	{
		return "08:00-18:00";
				...
				if($table=="vacancia_pendiente" && $field=="salario_final")
	{
		return $_SESSION['salario'];
	}
	return "";
}
