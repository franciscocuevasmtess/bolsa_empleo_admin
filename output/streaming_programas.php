...
$searchCondition = '';
if (!empty($searchTerm)) {
    $searchCondition = "WHERE nombre ILIKE '%$searchTerm%'";
}

// Consulta para obtener el total de registros (para la paginación)
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM bolsa_empleo.programas $searchCondition";
...
$query = "SELECT id_programas as id, nombre as text FROM bolsa_empleo.programas $searchCondition ORDER BY nombre LIMIT $records_per_page OFFSET $offset";
