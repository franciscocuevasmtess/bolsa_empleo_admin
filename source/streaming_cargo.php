...
$searchCondition = '';
if (!empty($searchTerm)) {
    $searchCondition = "WHERE descripcion_cargo ILIKE '%$searchTerm%'";
}

// Consulta para obtener el total de registros (para la paginación)
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM bolsa_empleo.cargo $searchCondition";
...
$query = "SELECT id_cargo as id, descripcion_cargo as text FROM bolsa_empleo.cargo $searchCondition ORDER BY descripcion_cargo LIMIT $records_per_page OFFSET $offset";
