...
$searchCondition = '';
if (!empty($searchTerm)) {
    $searchCondition = "WHERE descripcion_salario ILIKE '%$searchTerm%'";
}

// Consulta para obtener el total de registros (para la paginación)
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM bolsa_empleo.vacancia_salario_descripcion $searchCondition";
...
$query = "SELECT id_va_salario_desc as id, descripcion_salario as text FROM bolsa_empleo.vacancia_salario_descripcion $searchCondition ORDER BY descripcion_salario LIMIT $records_per_page OFFSET $offset";
